...
from .modeling_bert import ProteinBertForSequenceClassification
...
class BertForSequenceClassification(LightningModule):
    ...
    def __init__(self, model_config_file, lr, downstream_hid_dim, downstream_num_labels, downstream_dropout, from_pretrained=None, warmup_steps=0):
        """Inits model

        Args:
            model_config_file: Path to config file
            downstream_hid_dim: Downstream hidden layer dimensionality
            downstream_num_labels: Downstream number of classes
        ...
        self.model = ProteinBertForSequenceClassification(config, downstream_hid_dim, downstream_num_labels, downstream_dropout)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if 'label_weights' in batch:
            outputs = self.model(x,mask,y, weights = batch['label_weights'])
        else:
            outputs = self.model(x,mask,y)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if isinstance(outputs[0],tuple):
            ...
            loss = outputs[0]
            metrics = {}
        return {'val_loss': loss, 'logits': outputs[1], 'targets': y}
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.trainer.use_ddp:
            ...
            print(logits_all)
            """
            acc = accuracy(logits_all, targets_all)
            print('accuracy', acc)
            predictions = logits_all.float().argmax(-1)
            print('corrected', (predictions==targets_all).sum(), len(targets_all))
            logits_all = logits_all.cpu().detach().numpy()
            targets_all = targets_all.cpu().detach().numpy().squeeze()
            out = softmax(logits_all, axis=1)[:,1].squeeze()

            fpr, tpr, thresholds = sklearn.metrics.roc_curve(targets_all,out) 
            auroc = sklearn.metrics.auc(fpr, tpr)
            print('auroc', auroc)
            precision, recall, thresholds = sklearn.metrics.precision_recall_curve(targets_all,out)
